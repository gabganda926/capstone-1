...
{
    public function __construct(InstallmentType $inst)
    {
        $this->instal = $inst;
    }
    ...
    {
       $instal = InstallmentType::where('installment_ID', '=', $req->instid)->first();
    ...
    public function ardelete_installment(Request $req)
    {
      ...
      {
         $instal = InstallmentType::where('installment_ID', '=', $ID)->first();
         $instal->del_flag = 1;
         $mytime = $req->time;
         $instal->updated_at = $mytime;
